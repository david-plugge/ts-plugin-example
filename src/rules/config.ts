...
import { getSource, isPositionInsideNode, removeStringQuotes } from '../utils';
...
function createAutoCompletionOptionName(
	{ ts }: Context,
	sort: number,
	name: string,
) {
...
export const config = {
	...
	addCompletionsAtPosition(
		...
		visitEntryConfig(
			...
			(entryConfig, declaration) => {
				...
				context.log(
					`found '${entryConfig}' ${JSON.stringify(
						API_DOCS[entryConfig]?.options,
					)}`,
				);
				...
				prior.entries.push(
					...Object.keys(API_DOCS[entryConfig]?.options || {}).map(
						(name, index) => {
							return createAutoCompletionOptionValue(
								context,
								index,
								name,
								entryConfig,
	...
	getQuickInfoAtPosition(context: Context, fileName: string, position: number) {
		const { ts } = context;
