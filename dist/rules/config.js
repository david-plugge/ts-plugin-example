...
function visitEntryConfig({ ts, info }, fileName, position, callback) {
    const source = (0, utils_1.getSource)(info.languageService, fileName);
    if (source) {
        ts.forEachChild(source, function visit(node) {
            // Covered by this node
            if ((0, utils_1.isPositionInsideNode)(position, node)) {
                // Export variable
                if (ts.isVariableStatement(node) &&
                    node.modifiers?.some((m) => m.kind === ts.SyntaxKind.ExportKeyword)) {
                    if (ts.isVariableDeclarationList(node.declarationList)) {
                        for (const declaration of node.declarationList.declarations) {
                            if ((0, utils_1.isPositionInsideNode)(position, declaration)) {
                                // `export const ... = ...`
...
function createAutoCompletionOptionName({ ts }, sort, name) {
    return {
        name,
        sortText: '!' + sort,
...
function createAutoCompletionOptionValue({ ts }, sort, name, apiName) {
    const isString = name.startsWith('"');
    return {
        name,
        insertText: (0, utils_1.removeStringQuotes)(name),
        sortText: '' + sort,
...
exports.config = {
    ...
    addCompletionsAtPosition(context, fileName, position, prior) {
        visitEntryConfig(context, fileName, position, (entryConfig, declaration) => {
            if (!API_DOCS[entryConfig]) {
                context.log('not found');
                if ((0, utils_1.isPositionInsideNode)(position, declaration.name)) {
                    prior.entries.push(...Object.keys(API_DOCS).map((name, index) => {
                        return createAutoCompletionOptionName(context, index, name);
                    }));
                }
                return;
            }
            context.log(`found '${entryConfig}' ${JSON.stringify(API_DOCS[entryConfig]?.options)}`);
            prior.entries.push(...Object.keys(API_DOCS[entryConfig]?.options || {}).map((name, index) => {
                return createAutoCompletionOptionValue(context, index, name, entryConfig);
            }));
        });
    },
    // Show docs when hovering on the exported configs.
    getQuickInfoAtPosition(context, fileName, position) {
        const { ts } = context;
        let overriden;
        visitEntryConfig(context, fileName, position, (entryConfig, declaration) => {
            if (!API_DOCS[entryConfig])
            ...
            if (value && (0, utils_1.isPositionInsideNode)(position, value)) {
                // Hovers the value of the config
                const isString = ts.isStringLiteral(value);
                const text = (0, utils_1.removeStringQuotes)(value.getText());
                ...
                if (isValid) {
                    overriden = {
                        ...
                        documentation: [
                            {
                                kind: 'text',
                                text: API_DOCS[entryConfig]?.options?.[key] ||
    ...
    getCompletionEntryDetails(context, entryName, data) {
        const { ts } = context;
        if (data &&
    ...
    getSemanticDiagnosticsForExportVariableStatement(context, source, node) {
        const { ts } = context;
        const diagnostics = [];
        ...
        if (ts.isVariableDeclarationList(node.declarationList)) {
            for (const declaration of node.declarationList.declarations) {
                ...
                if (ts.isIdentifier(name)) {
                    if (!constants_1.ALLOWED_EXPORTS.includes(name.text) && !API_DOCS[name.text]) {
                        diagnostics.push({
                            file: source,
                            category: ts.DiagnosticCategory.Error,
                            code: constants_1.NEXT_TS_ERRORS.INVALID_ENTRY_EXPORT,
                        ...
                        if (value && options) {
                            ...
                            if (ts.isStringLiteral(value) ||
                                ...
                                const val = '"' + (0, utils_1.removeStringQuotes)(value.getText()) + '"';
                                ...
                                const v = value.getText();
                                if (!API_DOCS[name.text]?.isValid?.(JSON.stringify(value.elements.map((e) => e.getText())))) {
                                    isInvalid = true;
                                    displayedValue = v;
                                }
                            }
                            ...
                            if (isInvalid) {
                                diagnostics.push({
                                    file: source,
                                    category: ts.DiagnosticCategory.Error,
                                    code: constants_1.NEXT_TS_ERRORS.INVALID_OPTION_VALUE,
                    ...
                    else if (name.text === constants_1.LEGACY_CONFIG_EXPORT) {
                        // export const config = { ... }
                        // Error if using `amp: ...`
                        const value = declaration.initializer;
                            ...
                            for (const prop of value.properties) {
                                if (ts.isPropertyAssignment(prop) &&
                                    ...
                                    diagnostics.push({
                                        file: source,
                                        category: ts.DiagnosticCategory.Error,
                                        code: constants_1.NEXT_TS_ERRORS.INVALID_CONFIG_OPTION,
